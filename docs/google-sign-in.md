...
Kaisola intentionally does not ship the Desktop OAuth `client_secret`.
Google's installed-app token exchange marks that field optional because a
distributed native app cannot keep it confidential. The Desktop client ID,
loopback redirect, unique state and nonce, and S256 PKCE verifier are sufficient
for this public-client flow. A local `electron/google-oauth.json` remains
supported for backwards compatibility but is never required or generated by
